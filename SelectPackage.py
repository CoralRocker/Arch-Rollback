...
def main(stdscr):
    ...
    while True:
        ...
        for index, pkg in enumerate(pkg_list, 1):
            ...
            stdscr.addstr(str(pkg), (curses.A_REVERSE if current_item == index-1 or current_item in multiselect_indeces[key] else 0))
        
        stdscr.addstr(height - 1, 0, str(multiselect_indeces))
        c = stdscr.getch()
        if chr(c).lower() == 'q':
            break
